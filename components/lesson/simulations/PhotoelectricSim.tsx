...
export function PhotoelectricSim({ onComplete }: { onComplete: (success: boolean) => void }) {
  ...
  return (
    <div className="space-y-6">
      {/* Simulation Area */}
      <div className="card-3d bg-slate-900 border-slate-800 p-6 h-64 md:h-[400px] relative overflow-hidden flex items-end justify-center">
        
        {/* Light Beam (Color changes with frequency) */}
        <div 
          className="absolute top-[-20%] left-[-10%] w-full h-full origin-top-left -rotate-45 transition-colors duration-500"
          style={{
